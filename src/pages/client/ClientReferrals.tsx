...
export default function ClientReferrals() {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try { const c = await getClientByLink(accessLink); if (c) setClient(c); }
  ...
  const share = () => {
    const msg = encodeURIComponent(`Hola! Me apunto a Mundo Prana Viandas, viandas saludables. Usá mi código ${client.codigoReferido} cuando te suscribas y los dos ganamos puntos!`);
    ...
    <div className="space-y-4">
      ...
      <div className="glass-card p-5 text-center space-y-4">
        <p className="text-sm text-muted-foreground">
          Compartí tu código y ganás <strong>{PUNTOS_POR_REFERIDO} puntos</strong> por cada persona que se sume con tu código
        </p>
        <div className="text-3xl font-bold font-mono tracking-widest text-primary">{client.codigoReferido}</div>
        <div className="flex gap-3 justify-center">
          <Button variant="outline" onClick={copy}><Copy className="w-4 h-4 mr-2" />Copiar código</Button>
          <Button onClick={share}><Send className="w-4 h-4 mr-2" />Compartir por WhatsApp</Button>
        </div>
      </div>
      ...
      <div className="glass-card p-5 space-y-3">
        ...
        <ol className="space-y-2 text-sm text-muted-foreground">
          ...
          <li className="flex gap-2">
            ...
            Esa persona da tu código al inscribirse en Mundo Prana
          </li>
          <li className="flex gap-2">
