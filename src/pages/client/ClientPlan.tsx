...
export default function ClientPlan() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!client) return;
        const activePlan = (await getActivePlan(client.id)) || null;
        setPlan(activePlan);
  ...
  if (!plan) return <div className="text-center py-12 text-muted-foreground">Sin contrato activo</div>;
      ...
      <div className="glass-card p-5 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3 text-sm">
          ...
          {plan.diasFijos?.length ? (
            <div className="col-span-2"><span className="text-muted-foreground">Días fijos:</span> <strong>{plan.diasFijos.join(', ')}</strong></div>
          ) : null}
          {(plan.fechasExcluidas || []).length > 0 ? (
            <div className="col-span-2"><span className="text-muted-foreground">Fechas excluidas:</span> <strong>{plan.fechasExcluidas?.join(', ')}</strong></div>
          ) : null}
          {plan.horaLimite ? (
            <div className="col-span-2"><span className="text-muted-foreground">Hora límite de pedido:</span> <strong>{plan.horaLimite}</strong></div>
          ) : null}
          {plan.observaciones ? (
            <div className="col-span-2"><span className="text-muted-foreground">Observaciones:</span> <strong>{plan.observaciones}</strong></div>
          ) : null}
        </div>
      </div>
    </div>
